...
Current session update: second tag-triggered Buildkite release attempt for `v0.0.47` reached the release command with `BUILDKITE_TAG`, then failed because `tart-ci` also did not propagate signing/notary secrets into the guest: `MACOS_DEVELOPER_ID_CERT_P12_BASE64 is required`. Patched the release step to export all required release inputs via Buildkite interpolation inside the guest command (`BUILDKITE_TAG`, `MACOS_DEVELOPER_ID_CERT_P12_BASE64`, `MACOS_DEVELOPER_ID_CERT_PASSWORD`, `APPLE_NOTARY_KEY_ID`, `APPLE_NOTARY_ISSUER_ID`, `APPLE_NOTARY_KEY_P8_BASE64`) with xtrace disabled. Also changed `scripts/release-macos-notifier-buildkite.sh` preflight to report all missing required env vars at once. Next action: commit/push and recreate `v0.0.47` again.
