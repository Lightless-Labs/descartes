...
- [ ] **S3-priv** Opt-in elevated read path — **APPROVED 2026-07-10, validate in Linux CI.** Install-time privilege only, daemon **never** self-escalates; a tiny fixed-purpose `root_helper` (Rust) confines root/CAP_SYS_PTRACE, hardened with seccomp-bpf/PR_SET_NO_NEW_PRIVS/cap-drop in its own build pipeline; two-condition AND opt-in (OS grant present + config enabled, default off); OS-level audience-scoping (0750 `root:descartes-provenance` group / macOS XPC `csreq`); config/helper-path trust-boundary check; echo-back scope-verification on untrusted helper output; graceful degrade. `mechanism:"auto"` never targets `root_helper`.
  ...
  - **CHECKPOINT — S3 (first Rust crate) gated on confirming CI capability (operator: "confirm CI capability first; probably no passwordless sudo").** Repo-config finding: the Linux CI image `ci-linux-arm64-rust-bazel` is a NAME only — the HANDOFF's own review flags "Rust/Bazel substrate remains documentary" and the Linux job today only runs Node; it's `linux-arm64` (Tier-1 is x86_64); passwordless sudo on the Linux guest is unconfirmed (macOS guest has it). Ready-to-run probe: `scripts/probe-linux-ci-capability.sh` (reports rust/sudo/setcap/useradd/ptrace_scope). Run it in a one-off CI step (mirror the `linux-arm64` job's agents/plugins, `command: bash scripts/probe-linux-ci-capability.sh`) or by SSHing a guest. If rust/sudo/setcap are absent, S6 validation needs the `Lightless-Labs/tart-ci` Linux image extended before the Rust crate is worth building.
