...
Current session update: Buildkite #32 reached the Tart-isolated release job and confirmed `DOPPLER_TOKEN` was present inside the guest, but failed before fetching secrets because the macOS base image does not have the Doppler CLI: `error: DOPPLER_TOKEN is set but doppler CLI is unavailable`. Patched `scripts/release-macos-notifier-buildkite.sh` to fetch Doppler release secrets via Doppler REST using Python stdlib and Basic auth (`token:`) instead of requiring the Doppler CLI. Local validation passed: `bash -n scripts/release-macos-notifier-buildkite.sh`, targeted package metadata test, full `npm test`, and `git diff --check`. Next action: commit/push, recreate `v0.0.47`, then inspect the new release job.
