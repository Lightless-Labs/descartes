...
Plan-aligned next step: take the next high-impact capability step toward behavior-aware operations/defense investigation: implement redacted/bounded process arguments, then `inspect_process` and `inspect_parent_tree`. This is tracked in `todos/2026-05-19-process-identity-lineage-tools.md`. Linux x86_64 validation remains open; rerun with Node 20.18.1+ LTS or Node 22.9.0+ and a writable npm `--prefix`, then later promote to Buildkite (`todos/2026-05-19-linux-ci-validation.md`). Roadmap note added: temporary VM/container cleanup is framed as a north-star behavior enabled by intent preservation, lifecycle signals, evidence-grounded recommendations, and policy-gated actions rather than as a hardcoded workflow.
