...
| `containers.js` | Docker, Podman, Colima, Lima, and Podman machine container/runtime/host inventory with deterministic QEMU/Apple Virtualization host-resource correlation where process hints match. |
| `vms.js` | Local VM runtime inventory, container-host correlation metadata, and VM-like process hints including QEMU, VMware, UTM, and Apple Virtualization. |
