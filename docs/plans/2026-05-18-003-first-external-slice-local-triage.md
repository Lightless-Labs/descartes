...
**Addendum:** 2026-05-19 — process identity and lineage tools were added: default process evidence now uses redacted/bounded args, and guarded model-led triage can call `inspect_process` and `inspect_parent_tree` for PID-level identity/provenance without exposing shell/coding tools.
