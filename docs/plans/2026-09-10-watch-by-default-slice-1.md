...
**RESOLVED by the operator (2026-09-14)** — items #1, #2, #5 below are decided:
- **#1 Statistical tier → JOINS default-on WATCH.** `peer.count_spike/drop` and `session.churn/count_drop` go default-on alongside the four novelty detectors: same safety posture (completeness-gated + cold-start), and "learn a normal count/pattern, alert on significant deviation" *is* the vision — an odd-hour peer-login burst or sessions suddenly killed are exactly the signals to surface out of the box. **Carry-forward caveat:** statistical detectors are more tunable/noise-prone than a discrete "new service appeared," so they likely want a calibration pass sooner — treat that as the first fast-follow, not a blocker for this slice.
- **#2 Naming → `baseline` (and keep `alerts watch` unchanged).** The learning/inventory surface is **`descartes baseline status`** (per-domain phase/readiness: `still learning N/6` · `watching` · `cannot establish`) and **`descartes baseline inspect [--domain <name>]`** (the currently-established set / learned-normal, recomputed live from the fact-history window — no cache, since the detectors are deliberately stateless). `descartes alerts watch` (the live alert-tail) stays as-is; no bare-`watch` top-level command is added, so the collision is avoided. Rename §5's proposed `watch status/inspect` to `baseline status/inspect` throughout implementation.
- **#5 Consent / existing-install migration → NONE NEEDED.** Deployment is effectively single-user (the owner's own machine), so default-on flips with no first-run notice, upgrade prompt, or consent ceremony. §3.5's three-case migration and the `learned disable` case-3 `updated_at` ambiguity are moot — just default WATCH on.

**Still open:** #3 (correction-hook timing), #4 (a lightweight idle-resource sanity pass — still worth doing for the default-on collectors, even single-user), #6 (the README "off by default" language needs the accurate rewrite: "structural watching is on by default; authoring/promotion stays opt-in").
