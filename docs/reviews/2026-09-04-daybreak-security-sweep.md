...
**Re-gate residuals (2026-09-04 daybreak re-gate of the implemented fixes) — deferred OBSERVABILITY follow-ups, NOT fabrication:** the fail-closed suppression in both cases is CORRECT (daybreak agrees choosing either marker would reintroduce fabrication); what is missing is a positive, observable signal that suppression happened.
- **F3 residual [MEDIUM] — silent census-contradiction suppression.** The order-independent "two in-tick census markers with differing classified state → downgrade the tick to unknown/invalid" fix is CLOSED for fabrication, but an append-capable attacker (fact-history tampering / an in-process producer bug — the trusted-fact-source boundary) can mute transient novelty one tick at a time by injecting a single contradictory census marker, with no tamper/integrity signal recorded. Follow-up: emit a positive census-contradiction integrity signal alongside the downgrade (parallel to canary.tampered) so the suppression is loud, not silent. Owner: a stat-detector observability slice.
- **F6 residual [MEDIUM] — silent unsupported-availability-signature muting + forward-compat footgun.** The `PEER_SIGNATURE_INVALID` sentinel correctly separates present-but-invalid from legacy-markerless (CLOSED for fabrication), but `PEER_AVAILABILITY_SIGNATURE_PATTERN` accepts only `v1:` — a `v2:`-shaped signature (an attacker-appended marker OR, more importantly, a FUTURE Descartes schema bump) silently mutes peer.count_spike for that tick, and a continuous v2 producer mutes it indefinitely without recording why (a self-inflicted future outage when the schema is next versioned). The one-tick recompute-to-0 observability blip on the invalid tick is the already-accepted regime-flip cost. Follow-up: keep failing closed on unknown semantics but make it OBSERVABLE (an unsupported-version/integrity signal) and preferably retain the last trusted statistics rather than recomputing to 0. Owner: same stat-detector observability slice + a forward-compat/versioning policy decision.

_F5 [HIGH] was NOT fully closed by round 1 (earlier-tick ambiguity + string-not-chronological "strictly earlier" still fabricated churn) — closed in round 2 (reject ambiguity in BOTH selected ticks; require `olderMs < newerMs`); see the commit._
