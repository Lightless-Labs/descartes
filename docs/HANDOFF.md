...
- `tools/descartes-cli/` — initial npm-style Descartes CLI scaffold.
  ...
  - `src/tools/` contains read-only evidence collectors for system overview, top processes with redacted/bounded args, process identity/lineage inspection, bounded temporal sampling/artifacts, network basics, service manager basics, bounded recent logs, container basics, VM basics, scheduled job basics, disks, deterministic findings, and a combined triage bundle.
...
- `todos/` — frontmatter-indexed work items for quick triage/sorting:
  ...
  - `2026-05-19-expand-local-investigation-tools.md` — add more local read-only collectors; process identity/lineage, temporal sampling, network basics, service manager basics, recent logs, container basics, VM basics, and scheduled job basics are complete, with certificates/time sync as later candidates.
  - `2026-05-19-vm-inventory-collector.md` — completed first VM inventory slice for Tart, Lima, Multipass, VirtualBox, and libvirt/virsh; future parity can add UTM, VMware, Parallels, direct QEMU correlation, Incus/LXD VM mode, Podman machine, Proxmox, and Xen.
...
5. Recommended next task: expand into certificates/time sync from `todos/2026-05-19-expand-local-investigation-tools.md`, improve VM deduplication/resource correlation, or close the Linux x86_64 validation gap.
...
Recommended next task: choose the next local read-only collector from `todos/2026-05-19-expand-local-investigation-tools.md`; likely high-value options are certificates or time sync. Alternatively improve VM deduplication/resource correlation or close the Linux x86_64 validation gap.
...
- Current checked command: `npm test` passes 98 Node test cases.
- Current checked command: direct local `collectProcessEvidence({ limit: 3 })` returns ok on macOS with `ps -axo ...`.
- Current checked command: `npm run pack:dry-run` includes README plus runtime `tools/descartes-cli/src` files (including `tools/network.js`, `tools/services.js`, `tools/logs.js`, `tools/containers.js`, `tools/vms.js`, and `tools/scheduled-jobs.js`) and excludes tests/local artifacts for v0.0.23.
...
- Completed implementation: process args redaction/bounding plus `inspect_process` / `inspect_parent_tree`, disk filesystem classification/noise reduction, no-evidence/no-diagnosis guard, temporal sampling, network basics, service manager basics, bounded recent logs, container basics, VM basics/parity, and scheduled job basics. Recommended next implementation is certificates/time sync, VM deduplication/resource correlation, or Linux x86_64 validation.
