...
- `todos/` — frontmatter-indexed work items for quick triage/sorting:
  ...
  - `2026-05-19-expand-local-investigation-tools.md` — add more local read-only collectors; process identity/lineage, temporal sampling, network basics, service manager basics, recent logs, and container basics are complete, with VMs as a later candidate.
...
5. Recommended next task: expand into `collect_vms` or scheduled jobs from `todos/2026-05-19-expand-local-investigation-tools.md`, or close the Linux x86_64 validation gap.
...
- Current checked command: `npm test` passes 78 Node test cases.
- Current checked command: direct local `collectProcessEvidence({ limit: 3 })` returns ok on macOS with `ps -axo ...`.
- Current checked command: `npm run pack:dry-run` includes README plus runtime `tools/descartes-cli/src` files (including `tools/network.js`, `tools/services.js`, `tools/logs.js`, and `tools/containers.js`) and excludes tests/local artifacts for v0.0.20.
