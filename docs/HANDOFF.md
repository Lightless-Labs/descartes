...
Current session update: latest `v0.0.47` release attempt reached p12 import inside the Tart guest but failed signing identity detection: `error: no Developer ID Application signing identity found in imported p12` and `0 valid identities found`. This likely means the p12 does not contain a Developer ID Application certificate plus matching private key, or the imported identity is not considered a valid codesigning identity in the guest keychain. Patched `scripts/release-macos-notifier-buildkite.sh` to print safe signing diagnostics on this failure: imported certificate common names plus `security find-identity -v -p codesigning` output, and an explicit note that a downloaded `.cer` converted to `.p12` without the original private key will not work. Next action: commit/push and recreate `v0.0.47` to collect better diagnostics.
...
Recommended next task: continue `docs/plans/2026-05-30-native-macos-notifications.md` / `todos/2026-05-30-native-macos-notifications.md` by seeding Buildkite signing/notarization secrets, confirming the macOS agent queue, and running a tag-triggered macOS notifier release. Then validate Gatekeeper behavior, first-run macOS permission attribution, Notification Center display name/icon, denied-permission behavior, and daemon-context delivery before making native delivery the default macOS desktop path.
