...
**Pending operator direction (gates the body rewrite):** the threat-model posture — whether off-host attestation becomes a hard precondition for everything above notify-only (strongest governance, but blocks the auto-promote/act future on first building the attestation layer), or the subsystem is explicitly scoped to a non-host-compromise threat model and says so loudly, or a staged combination.
