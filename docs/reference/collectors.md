...
Fact-history: on the daemon's hourly structural tick (gated behind the `learned.json` kill switch, like every other structural sub-collector), `factPointsFromScheduledJobsEvidence` (`fact-translators.js`) translates this census into `scheduled_job.presence` fact-points plus one `scheduled_job.census` marker per genuinely-enumerated tick (including a genuine zero-job tick). The persisted `entity_key` is a length-prefixed composite of (`kind`, `source`, a per-entry identity leg) — for `systemd_timer`/`launchd_scheduled_job`/`periodic_directory_entry` jobs the identity leg is the entry's own real per-entry identity (`unit`/`label`/`path`, sanitized); for `cron` jobs (which carry no per-entry name field at all — every line in one crontab file shares the same `path`) the identity leg is instead a domain-separated SHA-256 digest of the entry's stable content (`path`, `schedule`, `user`, bounded/redacted `command`), so two distinct lines in one crontab never collapse to one entity and the raw command text never lands in fact-history. `census_state` is `"complete"` only when the collector's own `summary.unavailable_count === 0` AND its envelope-level `truncated` flag is not `true` (which subsumes both the total-vs-returned count leg and every probe-level truncation), else `"partial"`. This collector emits fact-history only — no alert candidates; alerting on a novel scheduled job (persistence installation) is a separate, later slice.
