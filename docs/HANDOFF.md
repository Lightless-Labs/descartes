...
- Current checked command: `npm test` passes 31 Node test cases.
...
- Remaining validation gap: true Linux x86_64 behavior/CI. Linux arm64 validation with `$HOME/.local` prefix now passes on public v0.0.11: install, symlinked `descartes --version`/`--help`, ChatGPT/Codex `--no-open` login, model-led guarded triage, `fallback_used: false`, `collect_triage_evidence`, `actions_taken: []`, and ok system/process/disk envelopes. JSON output includes full process command lines, reinforcing the next process-args redaction/bounding task. Package/docs require Node 20.18.1+ LTS or Node 22.9.0+ and the Linux todo uses a writable `--prefix`; future Buildkite validation should use scoped CI secrets rather than personal credentials where possible.
