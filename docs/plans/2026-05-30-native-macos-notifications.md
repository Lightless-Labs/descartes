...
- Add repeatable build/release packaging without hidden local build steps; this is for maintainers/Buildkite, not users.
  ...
  - Intended automation shape: pushing a version tag triggers a Buildkite macOS release job on the macOS Buildkite host, not inside the Tart guest, so signing/notary secrets are available without guest env passthrough. The job imports signing material into an ephemeral keychain, builds, signs, notarizes, staples, verifies, computes checksums, and publishes a macOS-specific release artifact.
