...
Current session update: prepared dedicated infrastructure-agent validation briefs for Linux ARM64 and Linux x86_64 at `linux-arm64-validation-brief.md` and `linux-x86_64-validation-brief.md`. Both briefs target v0.0.30+, use Node.js 22.19.0+, avoid privileged/mutating actions, install into a temporary npm prefix, check installed collector docs, validate isolated-XDG no-auth failure, run clone `npm test`/pack dry-run, run sanitized direct collector smokes across the current tool surface, capture read-only platform capability snapshots, and define optional credentialed model-led triage summaries without raw report upload. `todos/2026-05-19-linux-ci-validation.md` links these briefs and frames x86_64 as the Tier-1 gap-closing run.
...
- `linux-arm64-validation-brief.md` — standalone infrastructure-agent brief for best-effort Linux ARM64 v0.0.30+ validation.
- `linux-x86_64-validation-brief.md` — standalone infrastructure-agent brief for Tier-1 Linux x86_64 v0.0.30+ validation.
