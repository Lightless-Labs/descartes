...
Current session update: third tag-triggered Buildkite release attempt for `v0.0.47` failed because all signing/notary env vars were still missing inside the Tart guest; inspecting `github.com/Bande-a-Bonnot/tart-ci#v0.1.1` showed the plugin has no env passthrough config and runs only the command text inside the VM. Patched the release step to run directly on the `ci-macos-apple-silicon` Buildkite host instead of inside Tart, so Buildkite secrets are available to `scripts/release-macos-notifier-buildkite.sh`; CI jobs still use Tart. The release script still uses a runtime-generated ephemeral keychain and deletes decoded cert/notary key files. Next action: commit/push and recreate `v0.0.47` again.
