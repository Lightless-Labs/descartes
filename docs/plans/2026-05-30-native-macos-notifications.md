...
**Addendum:** 2026-07-07 — **release signing/notarization milestone complete.** Buildkite #67 (`v0.0.47`) built, Developer ID-signed, notarized (Accepted), stapled, and Gatekeeper-verified the helper inside the Tart guest, publishing the stapled zip + sha256 as Buildkite artifacts. Final fixes beyond the G2-intermediate work: the intermediate must also be installed into `/Library/Keychains/System.keychain` (codesign/trustd does not reliably consult a session-modified custom keychain search list), and the in-guest `buildkite-agent artifact upload` is skipped without an agent token. Remaining plan work: real-host validation of the notarized helper (permission prompt attribution/persistence) and a user-facing delivery path (GitHub Release upload currently skipped without `GITHUB_TOKEN`).

**Addendum:** 2026-07-22 — **real-host validation surfaced (and fixed) the exact risk the 2026-07-07 addendum flagged.** On a Homebrew-installed `v0.0.48`, `descartes alerts notifications test` failed with `authorization error: Notifications are not allowed for this application` (`UNErrorCodeNotificationsNotAllowed`). Root cause: the Node adapter exec'd the helper's inner Mach-O (`…/DescartesNotifier.app/Contents/MacOS/DescartesNotifier`) directly, and macOS's `UNUserNotificationCenter` refuses authorization for a process it does not recognize as a LaunchServices-launched, registered app — independent of code-signing. Confirmed on the real host: launching the SAME bundle via `open -n <app>.app --args …` delivers the notification (no prompt — permission persists under the stable bundle id). Fix (`notification-delivery.js`, Node-only, no notifier rebuild): a helper resolved inside a `.app` is now launched via `/usr/bin/open -g -n <app> --args …` (`macosAppBundleFor` detects the bundle); a bare-binary `--helper` override keeps the direct-exec path. Delivery status via `open` is **best-effort** (`delivery_confidence:"best_effort"`) because `open` returns a nonzero exit even on success (the accessory helper exits before `open -W` can wait on it — `kevent … No such process`), so a numeric-exit rejection from an `open` launch is treated as delivered, and only a spawn failure (string `error.code`) / timeout is a real error. **Deferred follow-up:** accurate post-delivery status would require the notifier to write its outcome to a result file the adapter reads — a re-signed release build, not done here. Shipped in `v0.0.49`. **VALIDATED on a real host (2026-07-22):** on Homebrew `0.0.49`, `descartes alerts notifications test --json` delivered via `open -g -n` and the notification **banner appeared** — closing this plan's last-open external validation criterion (first-run/real-host native delivery). `open -g -n` returned exit 0 (confident success path, not best-effort).

**Addendum:** 2026-07-09 — local implementation and release/delivery automation are complete through the Homebrew path, but the remaining unchecked validation criterion below is deliberately still external: first-run TCC/Notification Center attribution, display behavior, denied-permission behavior, and daemon-context delivery must be recorded from a real Mac before native delivery can become the default macOS desktop channel. Local Homebrew install/linkage/helper packaging validation passed after tap commit `75e886f`; see `docs/reviews/2026-07-09-homebrew-notifier-install-validation.md`. Use `macos-notifier-release-validation-brief.md` plus `scripts/validate-macos-notifier-helper.sh`; use `scripts/check-homebrew-tap-token.sh` before the next tag to reduce tap-bump token risk.
...
- Add repeatable build/release packaging without hidden local build steps; this is for maintainers/Buildkite, not users.
  ...
  - Current automation shape: pushing a version tag triggers a Buildkite macOS release job inside an ephemeral Tart guest using `github.com/Lightless-Labs/tart-ci#v0.2.4`. The job fetches release secrets inside the guest from Doppler, imports signing material into an ephemeral keychain plus required Developer ID intermediate/system certificate state, builds, signs, notarizes, staples, verifies, computes checksums, publishes GitHub Release assets, and attempts a best-effort Homebrew tap bump.
  - Current automation files: `.buildkite/pipeline.yml` and `scripts/release-macos-notifier-buildkite.sh`.
  - Required secret path: Buildkite exposes only the scoped Doppler token (`DOPPLER_DESCARTES_PRD_NOTARISATION`) to the Tart guest as `DOPPLER_TOKEN`; the guest fetches `MACOS_DEVELOPER_ID_CERT_P12_BASE64`, `MACOS_DEVELOPER_ID_CERT_PASSWORD`, `APPLE_NOTARY_KEY_ID`, `APPLE_NOTARY_ISSUER_ID`, `APPLE_NOTARY_KEY_P8_BASE64`, and `GITHUB_TOKEN` from Doppler. `HOMEBREW_TAP_GITHUB_TOKEN` is optional for a narrower tap-bump token. `CODESIGN_IDENTITY` is optional override-only; the script auto-detects the first Developer ID Application identity imported from the `.p12`.
- Keep the signed/notarized `.app` out of the cross-platform npm payload; deliver it via macOS-specific release asset, Homebrew formula resource, or an explicit macOS-only setup/download/install flow if npm users later need it.
...
- [x] Buildkite release secrets are seeded and the tag-triggered release flow signs/notarizes/verifies a real artifact before publication (validated for `v0.0.47`; first live tap-bump validation is tracked separately in `todos/2026-07-08-macos-release-validation.md`).
