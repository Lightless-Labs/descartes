...
- `linux-daemon-lifecycle-validation-brief.md` — standalone infrastructure-agent brief for Linux systemd-user daemon lifecycle validation (`install/start/status/history/stop/uninstall`) against v0.0.37+.
...
- `docs/plans/2026-05-23-daemon-history-store.md` — **recommended next plan** for installing/running a local background daemon and bounded local history/metric store.
...
- `todos/` — frontmatter-indexed work items for quick triage/sorting:
  ...
  - `2026-05-23-daemon-history-store.md` — **active task, in progress**: local background daemon with bounded XDG-owned metric/history persistence and CLI history summaries. Foreground loop, JSONL history, idempotent daemon lifecycle commands, and macOS install/start/history validation exist; Linux systemd-user validation, compact history UX, and richer `triage --use-history` behavior remains.
...
5. Recommended next task: continue `todos/2026-05-23-daemon-history-store.md` after the foreground daemon/history summary plus idempotent daemon lifecycle slice. Next useful steps are real-host launchd/systemd lifecycle validation, then richer history-aware triage prompting/evaluation and configurable retention/rollups.
...
Recommended next task: continue `docs/plans/2026-05-23-daemon-history-store.md` / `todos/2026-05-23-daemon-history-store.md`. The first foreground loop, compact history summary, idempotent daemon lifecycle slice, and initial `triage --use-history` path exist; do real-host launchd/systemd validation next, then richer history-aware triage prompting/evaluation and configurable retention/rollups.
