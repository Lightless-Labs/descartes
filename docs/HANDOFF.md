...
6. Recommended next task: wire/validate real macOS signing/notarization credentials and a macOS-specific release/install path for the native notifier, then validate Gatekeeper behavior, first-run permission attribution, Notification Center display name/icon, denied-permission behavior, and daemon-context delivery before making native delivery the default macOS desktop path.
...
Recommended next task: continue `docs/plans/2026-05-30-native-macos-notifications.md` / `todos/2026-05-30-native-macos-notifications.md` by wiring/validating real Developer ID signing/notarization credentials and a macOS-specific release/install path for the native helper. Then validate Gatekeeper behavior, first-run macOS permission attribution, Notification Center display name/icon, denied-permission behavior, and daemon-context delivery before making native delivery the default macOS desktop path.
...
- Current checked command: targeted native packaging/notification tests pass with `bash -n scripts/build-macos-notifier.sh scripts/notarize-macos-notifier.sh && node --test tools/descartes-cli/test/package-metadata.test.js tools/descartes-cli/test/notification-delivery.test.js`.
- Current checked command: `npm test` passes 181 Node test cases for v0.0.45.
...
- Current checked command: `npm pack --dry-run --json` succeeds for v0.0.45 and confirms the cross-platform npm package includes runtime `src` files but excludes `tools/descartes-cli/native`, so no macOS helper source/app payload is shipped to Linux installs.
